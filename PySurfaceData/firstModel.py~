...
class MODEL(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.chla = nn.Parameter(torch.ones((1,1), dtype=torch.float32)*np.rand(), requires_grad=True)
        self.NAP = nn.Parameter(torch.ones((1,1), dtype=torch.float32)*np.rand(), requires_grad=True)
        self.CDOM = nn.Parameter(torch.ones((1,1), dtype=torch.float32)*np.rand(), requires_grad=True)
